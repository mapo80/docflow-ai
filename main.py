# =============================================================================
# MAIN ORCHESTRATOR (English comments for maintainers)
# -----------------------------------------------------------------------------
...
import os, io, time, json, re, uuid, mimetypes
from typing import List, Dict, Any, Optional, Callable
from fastapi import FastAPI, UploadFile, File, Form, Depends, Response, status, HTTPException, Header
...
from parse import convert_markdown_async, extract_words_with_bboxes_pdf, parse_with_ppstructure_async, build_markdown_from_pp
from llm import extract_fields_async, extract_fields
import metrics
import jobs
import clients as clients_pkg
from clients import *  # unified import as clients_pkg
...
# ---------------- Core processing ----------------
async def _process_request(data: bytes, filename: str, tpl_json: dict, req_id: str, emit: Callable[[str],None]|None=None) -> dict:  # noqa: C901
    ...
    manifest = {
        ...
        "policy": {
            ...
            "llm": {"n_ctx": LLM_N_CTX, "seed": LLM_SEED, "temperature": LLM_TEMPERATURE, "json_strict": bool(LLM_JSON_STRICT)}
        }
    }
    artifacts = {}

    if emit: emit("markdown_start")
    ...
    if is_digital_text:
        for p in pages_words:
            ...
            for w in p.get("words", []):
                ...
                tokens.append({"text": w.get("text",""), "page": p.get("page",1), "bbox":[x0/pw, y0/ph, x1/pw, y1/ph], "line_id": None})
    ...
    if PPSTRUCT_POLICY == "always":
        ...
        if markdown_has_table(markdown):
            ...
            pp_should_run_global = indexer.sniff_tables_from_tokens(tokens, PPSTRUCT_SNIFF_MIN_ROWS, PPSTRUCT_SNIFF_MIN_COLS, PPSTRUCT_SNIFF_COL_ALIGN_TOL)
    ...
    if (pp_should_run_global or need_pp_for_content) and PPSTRUCT_POLICY != "auto_pages":
        if emit: emit("pp_start")
        t_pp0 = time.time()
        clients_pkg._COUNTERS['pp'] += 1
        pages_blocks = await parse_with_ppstructure_async(data, filename, pages=None)
        t_pp = time.time() - t_pp0
        jlog("ppstructure_done", id=req_id, pages=len(pages_blocks), total_blocks=sum(len(pg.get("blocks",[])) for pg in pages_blocks))
    ...
    if use_single_pass:
        ...
        for key in schema.fields:
            ...
            results.append({"key": key, "value": val or None, "confidence": round(confidence,4), "bboxes": bboxes, "bbox_pages": pages})
            field_details[key] = {
                "mode": "single_pass", "llm_ms": t_llm, "confidence": round(confidence,4),
                "validation_status": "skipped", "retrieval": [], "prompt": schema.llm_text,
                "context": markdown[:4000] + ("... [truncated]" if len(markdown)>4000 else ""),
                "llm_raw": json.dumps(item, ensure_ascii=False, indent=2),
                "token_indices": tok_idx, "bboxes": bboxes, "bbox_pages": pages
            }
    else:
        for key in schema.fields:
            hits = idx.search(key, topk=RAG_TOPK)
            ctx_parts, retrieval_dump = [], []
            for idx_id, score in hits:
                ch = chunks[idx_id]; ctx_parts.append(ch["text"])
                retrieval_dump.append({"chunk_id": idx_id, "score": score, "kind": ch["kind"], "start": ch["start"], "len": len(ch["text"])})
            context = "\n\n".join(ctx_parts)
            t_llm0 = time.time()
            ...
            results.append({"key": key, "value": val or None, "confidence": round(confidence,4), "bboxes": bboxes, "bbox_pages": pages})
            field_details[key] = {
                "mode": "rag_field_wise", "llm_ms": t_llm, "confidence": round(confidence,4),
                "validation_status": "skipped", "retrieval": retrieval_dump, "prompt": schema.llm_text,
                "context": context, "llm_raw": json.dumps(item, ensure_ascii=False, indent=2),
                "token_indices": tok_idx, "bboxes": bboxes, "bbox_pages": pages
            }
    ...
    if os.getenv("DEBUG_OVERLAY","0") in ("1","true","yes"):
        matches_per_page = {}
        for item in results:
            key = item["key"]; bxs = item.get("bboxes", []) or []; pgs = item.get("bbox_pages", []) or [1]*len(bxs)
    ...
    response = {"request_id": req_id, "template": schema.name, "text": markdown, "fields": results, "debug_overlays": debug_files, "status": "done"}

    manifest.update({"timings_ms": {"markdown": int(t_markdown*1000), "pp": int(t_pp*1000)}, "ppstructure": {"policy": PPSTRUCT_POLICY}})
...
# ---------------- Routes ----------------
@app.post("/extract")
async def extract(file: UploadFile = File(...), template: str = Form(...), _auth_ok: bool = Depends(get_api_key)):
...
@app.post("/jobs")
async def submit_job(file: UploadFile = File(...), template: str = Form(...), priority: int = Form(5), _auth_ok: bool = Depends(get_api_key)):
    data = await file.read()
    try:
        tpl = json.loads(template)
    except Exception:
...
async def get_report_zip(request_id: str, _auth_ok: bool = Depends(get_api_key)):
    ...
    return Response(content=zbytes, media_type="application/zip", headers={"Content-Disposition": f"attachment; filename={request_id}.zip"})
